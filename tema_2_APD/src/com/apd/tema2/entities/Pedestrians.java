...
public class Pedestrians implements Runnable {
    ...
    @Override
    public void run() {
        // masinile sunt notificate de start si vor afisa fiecare culoarea initiala de la semafor(verde pt ele)
        ...
        while(System.currentTimeMillis() - startTime < executeTime) {
            try {
                ...
                if(pedestriansNo == maxPedestriansNo) {
                    pedestriansNo = 0;
                    pass = true;
                    // masinile sunt notificate de schimbarea culorii semaforului(rosu pt ele)
                    synchronized (Main.pedestrians) {
                        Main.pedestrians.notifyAll();
                    }
                    ...
                    pass = false;
                    // masinile sunt notificate de schimbarea culorii semaforului(verde pt ele)
                    synchronized (Main.pedestrians) {
                        Main.pedestrians.notifyAll();
                    }
                }
        ...
        finished = true;
        // masinile sunt notificate de sfarsit
        synchronized (Main.pedestrians) {
            Main.pedestrians.notifyAll();
        }
    }
